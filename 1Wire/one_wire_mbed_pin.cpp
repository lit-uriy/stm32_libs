...
OneWireMbedPin::LineStatus OneWireMbedPin::status()
{
    return _status;
}

int OneWireMbedPin::errorCode()
{
    return _errorCode;
}


OneWireRomCode OneWireMbedPin::findSingleDevice()
{
    _errorCode = ErrorNon;
    OneWireRomCode rom;
    OneWireMbedPin::LineStatus status = reset();
    if (status != OneWireMbedPin::StatusPresence){
...
bool OneWireMbedPin::findDevices(YList<OneWireRomCode*> *romList)
{
    printf("OneWire::findDevices()\n");
    bool ok = true;
    OneWireMbedPin::LineStatus status = findMultipleDevices(romList);
...
OneWireMbedPin::LineStatus OneWireMbedPin::findMultipleDevices(YList<OneWireRomCode*> *romCodes)
{
...
void OneWireMbedPin::addDevice(OneWireDevice *dev)
{
    _devices.append(dev);
    dev->_wire = this;
}

void OneWireMbedPin::removeDevice(OneWireDevice *dev)
{
...
YList<OneWireDevice *> OneWireMbedPin::devices()
{
    return _devices;
}

OneWireMbedPin::LineStatus OneWireMbedPin::reset()
{
    _errorCode = ErrorNon;
...
bool OneWireMbedPin::readROM(OneWireRomCode *romCode)
{
    ...
    if (reset() != OneWireMbedPin::StatusPresence){
...
OneWireMbedPin::LineStatus OneWireMbedPin::searchROM(OneWireRomCode *romCode, bool next)
{
...
bool OneWireMbedPin::matchROM(const OneWireRomCode romCode)
{
    int i;
    _errorCode = ErrorNon;
...
//        syncroPin.write(0);
    ...
    unsigned char temp = OneWireMbedPin::CommandMatchRom;
    if (readWriteByte(&temp) != OneWireMbedPin::StatusPresence){
...
OneWireMbedPin::LineStatus OneWireMbedPin::readWriteByte(unsigned char *byte)
{
    unsigned char j;
    _errorCode = ErrorNon;
...
OneWireMbedPin::LineStatus OneWireMbedPin::readWriteBit(bool *bit)
{
    _errorCode = ErrorNon;
